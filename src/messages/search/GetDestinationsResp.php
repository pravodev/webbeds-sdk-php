...
use Webbeds\HotelApiSdk\Model\Search\DestinationIterator;

/**
 * Class DestinationResp
 * @package Webbeds\HotelApiSdk\Messages
 * @property Destinations roomNoteTypes used for hotel content
 */
class GetDestinationsResp extends ApiResponse
{
    /**
    ...
    public function __construct(array $rsData)
    {
        ...
        if (array_key_exists("Destinations", $rsData)) {
            //$roomNoteTypesObject = new Destinations($this->roomNoteTypes);
            //$this->roomNoteTypes = $roomNoteTypesObject;
            $this->destinations = $rsData['Destinations']['Destination'];
        }
    }
    /**
    ...
    {
        return (count( $this->destinations)=== 0);
    }

    public function iterator()
    {
        if ($this->destinations !== null)
            return new DestinationIterator($this->destinations);
        return new DestinationIterator([]);
    }
